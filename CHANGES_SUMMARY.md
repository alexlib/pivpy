...
- **(2026-08)** Storage: added Zarr as a first-class format (`save_piv(..., format="zarr")`, `read_piv(path)` auto-detects `.zarr`, `read_directory_lazy()`/`convert_directory_to_zarr()` for out-of-core directories). **(2026-08, later same day)** `save_piv`'s default `format` flipped from `"netcdf"` to `"zarr"` — now that `netcdf4` is an optional extra, `"netcdf"` could no longer be assumed always available, while `zarr`/`dask` are hard dependencies. `save_piv(ds, path)` with no `format=` now writes a Zarr store (a directory) at `path`, not a single `.nc` file; pass `format="netcdf"` explicitly for the old behavior. See `docs/architecture/zarr-migration.md` for the full plan.
...
- **(2026-08)** Dependencies: `netcdf4` and `vortexfitting` are no longer hard dependencies (moved to the `netcdf`/`vortexfitting` extras and `full`). Default NetCDF I/O now goes through `h5netcdf` (built on `h5py`, still a hard dependency) instead of the `netcdf4` C-extension package. `interfacing.pivpyTOvf()` raises a clear `ImportError` if `vortexfitting` isn't installed, instead of a `NameError` from a module-level import warning. This keeps the base install installable on Python builds without wheels for those C extensions yet (verified on a free-threaded/no-GIL CPython 3.14 build).
- **(2026-08)** `piv.__mul__`/`piv.__truediv__` (renamed from the Python-2-only, never-actually-called `__div__`) no longer mutate the caller's Dataset in place — both return a new Dataset. Scalar-producing accessor methods (`vorticity`, `strain`, `divergence`, `acceleration`, `kinetic_energy`, `tke`, `reynolds_stress`, `rms`) now emit a `UserWarning` when their default `name="w"` is about to overwrite an existing variable (still allowed, now visible). `crop()`/`vec2scal()` emit a `DeprecationWarning` about their internal `self._obj` reassignment ahead of a future release that removes it.
