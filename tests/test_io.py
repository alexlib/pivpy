...
def test_get_frame():
    ...
    _, _, _, _,_,frame = io.parse_header(os.path.join(path,'day2/day2a005003.T000.D000.P003.H001.L.vec'))
    assert frame == 5003
    _, _, _, _,_,frame = io.parse_header(os.path.join(path,'Insight/Run000002.T000.D000.P000.H001.L.vec'))
    assert frame == 2
    _, _, _, _,_,frame = io.parse_header(os.path.join(path,'openpiv/exp1_001_b.vec'))
    assert frame == 1 
    _, _, _, _,_,frame = io.parse_header(os.path.join(path,'openpiv/exp1_001_b.txt'))
    assert frame == 1   
...
def test_get_units():
    ...
    lUnits,vUnits,tUnits = io.get_units(os.path.join(path,'day2/day2a005000.T000.D000.P003.H001.L.vec'))
    assert lUnits == 'pixel'
...
def test_load_directory():
    ...
    data = io.load_directory(path=os.path.join(path,'urban_canopy'),\
        basename='B*',ext='.vc7')
    assert np.allclose(data['t'],[0,1,2,3,4])
